...
import com.incomm.cclp.dao.SpilCreditDAO;
import com.incomm.cclp.dto.ValueDTO;
import com.incomm.cclp.exception.ServiceException;
import com.incomm.cclp.util.Util;

@Repository
public class SpilCreditDAOImpl implements SpilCreditDAO {
	@Autowired
	JdbcTemplate jdbcTemplate;

	private static final Logger logger = LogManager.getLogger(SpilCreditDAOImpl.class);
	...
	public String[] invoke(ValueDTO valueDto) throws ServiceException {
		...
		try {
			...
			params.add(new SqlOutParameter("AccountPurseId", Types.NUMERIC));

			Map<String, Object> resultMap = jdbcTemplate.call(new CallableStatementCreator() {
				public CallableStatement createCallableStatement(Connection con) throws SQLException {

					CallableStatement cs = con.prepareCall(QueryConstants.SP_SPIL_CREDIT); // need to call proper proce
					...
					logger.info("calling.. " + QueryConstants.SP_SPIL_CREDIT);

					cs.setString(1, valueObj.get(ValueObjectKeys.CARD_NUMBER));
					logger.info(ValueObjectKeys.CARD_NUMBER + ":" + valueObj.get(ValueObjectKeys.CARD_NUMBER));
					...
					cs.setString(39, ValueObjectKeys.FLAG_NO);
					...
					cs.setString(54, valueObj.get(ValueObjectKeys.STORE_ID));
			...
			result[5] = String.valueOf(resultMap.get("AccountPurseId"));
			logger.info("spil credit response responseCode:{},responsemsg:{},AuthorizedId:{},Balance:{},AuthorizedAmt:{},CurrencyCode:{}",
					result[0], result[1], result[2], result[3], result[4], result[5]);
		} catch (Exception e) {
			logger.error("Error Occured in {} procedure. error message {}", QueryConstants.SP_SPIL_CREDIT, e.getMessage(), e);
