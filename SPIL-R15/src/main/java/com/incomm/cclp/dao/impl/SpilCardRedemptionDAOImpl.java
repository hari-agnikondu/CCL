...
import com.incomm.cclp.dao.SpilCardRedemptionDAO;
import com.incomm.cclp.dto.ValueDTO;
import com.incomm.cclp.exception.ServiceException;
import com.incomm.cclp.util.Util;

@Repository
public class SpilCardRedemptionDAOImpl implements SpilCardRedemptionDAO {
		...
		try {
			...
			Map<String, Object> resultMap = jdbcTemplate.call(new CallableStatementCreator() {
				public CallableStatement createCallableStatement(Connection con) throws SQLException {
					...
					CallableStatement cs = con.prepareCall(QueryConstants.CARDREDEMPTION); // need to call proper proce
					logger.info("Query:" + QueryConstants.CARDREDEMPTION);
					...
					logger.info(ValueObjectKeys.CARD_USAGE_LIMIT + ":" + "null");
			...
			logger.info("Balance" + ":" + result[3]);

			result[4] = String.valueOf(resultMap.get("AuthorizedAmt"));
			logger.info(ValueObjectKeys.AUTHORIZED_AMOUNT + ":" + result[4]);

			result[5] = valueObj.get(ValueObjectKeys.TXN_CURRENCY_CODE);
			logger.info(ValueObjectKeys.TXN_CURRENCY_CODE + ":" + result[5]);

			result[6] = String.valueOf(resultMap.get("PurseAuthResponse"));
			logger.info(" Purse Auth Response" + ":" + result[6]);

		} catch (Exception e) {
			logger.error("Error Occured in {} procedure. error message {}", QueryConstants.CARDREDEMPTION, e);
