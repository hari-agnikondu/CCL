...
import com.incomm.cclp.dao.SpilStoreCreditDAO;
import com.incomm.cclp.dto.ValueDTO;
import com.incomm.cclp.exception.ServiceException;
import com.incomm.cclp.util.Util;

@Repository
public class SpilStoreCreditDAOImpl implements SpilStoreCreditDAO {
		...
		try {
			...
			Map<String, Object> resultMap = jdbcTemplate.call(new CallableStatementCreator() {
				public CallableStatement createCallableStatement(Connection con) throws SQLException {
					...
					CallableStatement cs = con.prepareCall(QueryConstants.SP_SPIL_STORE_CREDIT); // need to call proper
																									// proce
					logger.info(QueryConstants.SP_SPIL_STORE_CREDIT);
					...
					logger.info("Activartion Date:" + activationdate);
					...
					cs.setString(54, valueObj.get(ValueObjectKeys.RETAIL_PATRNER_REFERENCE_NUMBER));
					...
					cs.registerOutParameter(62, Types.NUMERIC);
			...
			result[5] = String.valueOf(resultMap.get("AccountPurseId"));// p_account_purse_id
			logger.info(
					"Spil store credit response: responseCode: {}, responseMsg:{}, AuthorizedId: {}, balance:{}, AuthorizedAmt:{}, Currency Code:{}",
					result[0], result[1], result[2], result[3], result[4], result[5]);
		} catch (Exception e) {
			logger.error("Error Occured in {} procedure. error message {}", QueryConstants.SP_SPIL_STORE_CREDIT, e.getMessage(), e);
