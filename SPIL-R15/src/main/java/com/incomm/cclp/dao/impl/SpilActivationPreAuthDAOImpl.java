...
@Repository
public class SpilActivationPreAuthDAOImpl implements SpilActivationPreAuthDAO {

	@Autowired
	JdbcTemplate jdbcTemplate;

	private static final Logger logger = LogManager.getLogger(SpilActivationPreAuthDAOImpl.class);
	...
	public String[] invoke(ValueDTO valueDto) throws ServiceException {
		...
		try {
			...
			params.add(new SqlOutParameter("54", Types.NUMERIC));
			...
			Map<String, Object> resultMap = jdbcTemplate.call(new CallableStatementCreator() {
				public CallableStatement createCallableStatement(Connection con) throws SQLException {
					CallableStatement cs = con.prepareCall(
							" {call PKG_CARD_ACTIVATION.spil_activation_preauthorization(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
					logger.info("Query:"
							+ " {call PKG_CARD_ACTIVATION.spil_activation_preauthorization(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
					...
					cs.setString(50, valueObj.get(ValueObjectKeys.STORE_ID));
					...
					cs.registerOutParameter(54, Types.NUMERIC);
			...
			logger.error("Error in SpilActivationPreAuthDAOImpl: " + e.getMessage(), e);
