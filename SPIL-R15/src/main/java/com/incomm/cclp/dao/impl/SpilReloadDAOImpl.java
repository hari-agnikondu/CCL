...
@Repository
public class SpilReloadDAOImpl implements SpilReloadDAO {
	...
	public String[] invokeReload(ValueDTO valueDto) throws ServiceException {
		String[] respFields = new String[6];
		...
		try {
			...
			params.add(new SqlOutParameter("58", Types.NUMERIC));

			Map<String, Object> resultMap = jdbcTemplate.call(new CallableStatementCreator() {
				public CallableStatement createCallableStatement(Connection con) throws SQLException {
					CallableStatement cs = null;
					if (GeneralConstants.MSG_TYPE.equals(valueObj.get(ValueObjectKeys.MSGTYPE))) {
						cs = con.prepareCall(QueryConstants.SP_SPIL_TOPUP);
						logger.info("calling... {}", QueryConstants.SP_SPIL_TOPUP);
					} else {
						cs = con.prepareCall(QueryConstants.SP_SPIL_TOPUP_RVSL);
						logger.info("calling... {}", QueryConstants.SP_SPIL_TOPUP_RVSL);
					}
					...
					logger.info(ValueObjectKeys.SPIL_FEE_AMT + ":"
							+ (valueObj.get(ValueObjectKeys.SPIL_FEE_AMT) != null ? valueObj.get(ValueObjectKeys.SPIL_FEE_AMT) : "0.0"));
					cs.setBigDecimal(34, new BigDecimal(
							valueObj.get(ValueObjectKeys.SPIL_TRAN_AMT) != null ? valueObj.get(ValueObjectKeys.SPIL_TRAN_AMT) : "0.0"));
					...
					cs.setString(39, valueObj.get(ValueObjectKeys.SPIL_ZIP));
					logger.info(ValueObjectKeys.SPIL_ZIP + ":" + valueObj.get(ValueObjectKeys.SPIL_ZIP));
					// 13.06.2018 added
					cs.setString(40, valueObj.get(ValueObjectKeys.CASHIER_ID));
					logger.info(ValueObjectKeys.CASHIER_ID + ":" + valueObj.get(ValueObjectKeys.CASHIER_ID));
					cs.setString(41, valueObj.get(ValueObjectKeys.EXP_DATE));
					...
					if (valueObj.get(ValueObjectKeys.CURRENCY_CONV_RATE) != null) {
						cs.setBigDecimal(42, new BigDecimal(valueObj.get(ValueObjectKeys.CURRENCY_CONV_RATE)));
						logger.info(ValueObjectKeys.CURRENCY_CONV_RATE + ":" + valueObj.get(ValueObjectKeys.CURRENCY_CONV_RATE));
					} else {
						cs.setBigDecimal(42, null);
						logger.info(ValueObjectKeys.CURRENCY_CONV_RATE + ":" + "null");
					}

					cs.setString(43, valueObj.get(ValueObjectKeys.TXN_CURRENCY_CODE));
					logger.info(ValueObjectKeys.TXN_CURRENCY_CODE + ":" + valueObj.get(ValueObjectKeys.TXN_CURRENCY_CODE));
					cs.setBigDecimal(44, new BigDecimal(valueObj.get(ValueObjectKeys.ORGNL_TRAN_AMOUNT)));
					...
					if (!Util.isEmpty(valueObj.get(ValueObjectKeys.PURAUTHREQ_ACCOUNT_PURSE_ID))) {
						cs.setInt(45, Integer.parseInt(valueObj.get(ValueObjectKeys.PURAUTHREQ_ACCOUNT_PURSE_ID)));
					...
					cs.registerOutParameter(58, Types.NUMERIC);

					return cs;
				}
			}, params);

			respFields[0] = (String) resultMap.get("52");// p_resp_code
			respFields[1] = (String) resultMap.get("53");// p_err_msg
			respFields[2] = (String) resultMap.get("54");// p_auth_id
			respFields[3] = String.valueOf(resultMap.get("55"));// p_balance
			respFields[4] = String.valueOf(resultMap.get("57"));// p_authamount
			respFields[5] = String.valueOf(resultMap.get("58"));// p_account_purse_id
			logger.debug("Response message : " + respFields[1] + "Response Code : " + respFields[0] + "  Total amt:" + respFields[3]
					+ " AuthId " + respFields[2]);
		} catch (Exception e) {
			logger.error("Error in Spil Reload Dao: " + e.getMessage(), e);
