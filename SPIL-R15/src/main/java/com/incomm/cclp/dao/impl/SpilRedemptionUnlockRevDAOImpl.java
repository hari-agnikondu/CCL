...
@Repository
public class SpilRedemptionUnlockRevDAOImpl implements SpilRedemptionUnlockRevDAO {
		...
		try {
			...
			Map<String, Object> resultMap = jdbcTemplate.call(new CallableStatementCreator() {
				public CallableStatement createCallableStatement(Connection con) throws SQLException {
					...
					CallableStatement cs = con.prepareCall(QueryConstants.SP_SPIL_REDEMPTION_UNLOCK_REVERSAL); // need
																												// to
																												// call
																												// proper
																												// proce
					...
					logger.info(QueryConstants.SP_SPIL_REDEMPTION_UNLOCK_REVERSAL);
			...
			logger.info(
					"Redemption unlock respose: responseCode: {}, responseMsg:{}, AuthorizedId:{}, Balance:{}, AuthorizedAmt:{}, CurrencyCode:{}",
					result[0], result[1], result[2], result[3], result[4], result[5]);

		} catch (Exception e) {
			logger.error("Error Occured in {} procedure. error message {}", QueryConstants.SP_SPIL_REDEMPTION_UNLOCK_REVERSAL,
					e.getMessage(), e);
