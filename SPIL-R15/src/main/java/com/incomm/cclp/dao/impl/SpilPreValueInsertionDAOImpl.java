...
@Repository
public class SpilPreValueInsertionDAOImpl implements SpilPreValueInsertionDAO {

	private static final Logger logger = LogManager.getLogger(SpilPreValueInsertionDAOImpl.class);
	@Autowired
		...
		try {
			...
			Map<String, Object> resultMap = jdbcTemplate.call(new CallableStatementCreator() {
				public CallableStatement createCallableStatement(Connection con) throws SQLException {
					CallableStatement cs = con.prepareCall(QueryConstants.SP_SPIL_PRE_VALUE_INS);

					logger.info(QueryConstants.SP_SPIL_PRE_VALUE_INS);
			...
			respFields[3] = (String) resultMap.get("55");
			logger.debug("Response message : " + respFields[1] + "Response Code : " + respFields[0] + " Amount:" + respFields[3]
					+ "  AuthId:" + respFields[2]);
		} catch (Exception e) {

			logger.error("Exception in Spil PreValueInsertion Dao" + e.getMessage(), e);
			throw new ServiceException(SpilExceptionMessages.DAO_SQL_EXEC, ResponseCodes.SYSTEM_ERROR);
		}
