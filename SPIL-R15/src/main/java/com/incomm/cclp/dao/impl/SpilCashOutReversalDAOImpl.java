...
@Repository
public class SpilCashOutReversalDAOImpl implements SpilCashOutReversalDAO {

	@Autowired
	JdbcTemplate jdbcTemplate;

	private static final Logger logger = LogManager.getLogger(SpilCashOutReversalDAOImpl.class);
	...
	public String[] spilCashOutReversal(ValueDTO valueDto) throws ServiceException {
		...
		try {
			...
			params.add(new SqlOutParameter("59", Types.NUMERIC));

			Map<String, Object> resultMap = jdbcTemplate.call(new CallableStatementCreator() {
				public CallableStatement createCallableStatement(Connection con) throws SQLException {
					CallableStatement cs = con.prepareCall(
							"{call PKG_CARD_REDEEM.SPIL_CASHOUT_REVERSAL(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
					...
					cs.setString(4, valueObj.get(ValueObjectKeys.OLD_CARD_STATUS));
					...
					cs.setString(49, valueObj.get(ValueObjectKeys.PURSE_ID));
					cs.setInt(50, 0);
					cs.setInt(51, 0);

					cs.setString(52, valueObj.get(ValueObjectKeys.STORE_ID));
			...
			result[0] = (String) resultMap.get("53");
			result[1] = (String) resultMap.get("54");
			result[2] = (String) resultMap.get("55");
			result[3] = String.valueOf(resultMap.get("56"));
			result[4] = valueObj.get(ValueObjectKeys.TXN_CURRENCY_CODE);
			logger.info("Spil Cash out response: responseCode:{}, responseMsg:{}, Authorizedid:{}, authorizedAmt:{}, currencyCode:{}",
			...
			logger.error("Error in SpilCashOutReversalDAOImpl: " + e.getMessage(), e);
