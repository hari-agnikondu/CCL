...
@Repository
public class SpilCardRedemptionReversalDAOImpl implements SpilCardRedemptionReversalDAO {
		...
		try {
			...
			Map<String, Object> resultMap = jdbcTemplate.call(new CallableStatementCreator() {
				public CallableStatement createCallableStatement(Connection con) throws SQLException {
					...
					CallableStatement cs = con.prepareCall(QueryConstants.SP_SPIL_CARD_REDEMPTION_REVERSAL); // need to
																												// call
																												// proper
																												// proce
					...
					logger.info(QueryConstants.SP_SPIL_CARD_REDEMPTION_REVERSAL);
			...
			logger.info(
					"card redemption reversal response: responseCode:{}, responseMsg:{}, authorizedId:{}, balance:{}, authorizedAmt:{}, currencyCode:{} ,PurseAuthResponse:{}",
					result[0], result[1], result[2], result[3], result[4], result[5], result[6]);
		} catch (Exception e) {
			logger.error("Error Occured in {} procedure. error message {}", QueryConstants.SP_SPIL_CARD_REDEMPTION_REVERSAL, e.getMessage(),
					e);
