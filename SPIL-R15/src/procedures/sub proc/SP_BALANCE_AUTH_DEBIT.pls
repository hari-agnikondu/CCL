create or replace PROCEDURE sp_balance_auth_debit (
    p_prod_id                  IN NUMBER,
    p_last_txndate             DATE,
...
BEGIN
    ...
    EXCEPTION
        WHEN no_data_found THEN
            ...
            p_err_msg := 'ERROR NO DATA FOUND WHILE GETTING DEBIT BALANCE'
             || substr(sqlerrm,1,200);
            RAISE l_exception;
        WHEN OTHERS THEN
            p_resp_code := trans_const.system_error;
            p_err_msg := 'ERROR WHILE GETTING ACCOUNT BALANCE ON DEBIT'
             || substr(sqlerrm,1,200);
            RAISE l_exception;
    END;
    ...
    EXCEPTION
        ...
        WHEN OTHERS THEN
            p_resp_code := trans_const.system_error;
            p_err_msg := 'ERROR WHILE CHECKING BALANCE AUTH DEBIT- FEE CALC '
             || substr(sqlerrm,1,200);
            RAISE l_exception;
    END;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            p_resp_code := trans_const.system_error;
            p_err_msg := 'Updating LEDGER_BALANCE and AVAILABLE_BALANCEt ON DEBIT '
             || substr(sqlerrm,1,200);
            RAISE l_exception;
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        p_resp_code := trans_const.system_error;
        p_err_msg := 'ERROR WHILE CHECKING BALANCE AUTHENTICATION DEBIT'
         || substr(sqlerrm,1,200);
END;