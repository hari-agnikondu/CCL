create or replace PROCEDURE sp_spil_card_activation (
...
---Calling sp_authorize_transaction_new procedure
    ...
    -- UDPATE CARD STATUS,OLD CRAD STATUS,DATE OF ACTIVATION,LAST TXN AMOUNT AND TOP-UP FLAG IN CARD TABLE

        BEGIN
            UPDATE card
                SET
                    usage_limit = to_clob(p_usage_limit),
                    card_status = p_card_status,
                    old_cardstat = p_old_cardstatus,
                    date_of_activation = p_activation_date,
                    last_txndate = SYSDATE,
                    firsttime_topup = p_firsttime_topup
            WHERE
                card_num_hash = v_card_num_hash;
