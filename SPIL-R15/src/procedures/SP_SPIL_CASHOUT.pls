create or replace PROCEDURE SP_SPIL_CASHOUT
(
...
from product where product_id=p_prod_id;
 ...
 ---Calling sp_authorize_transaction_redemtion lock
...
---Calling sp_balance_auth_debit_cashout lock
    ...
    v_max_txncount_flag
        ...
        EXCEPTION
            ...
            WHEN OTHERS THEN
                p_resp_code := trans_const.system_error;
                p_err_msg := 'ERROR WHILE GETTING BALANCE AUTH DEBIT TRANSACTION '
                 || substr(sqlerrm,1,200);
                RAISE l_exception;
        END;
