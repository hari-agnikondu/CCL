create or replace PROCEDURE SP_SPIL_DEACTIVATION_REVERSAL
(   
...
from product where product_id=p_prod_id;
  ...
  EXCEPTION
    ...
    --Calling THE authorize TRANSACTION 
        BEGIN
   ...
   -- v_purse_id,
	...
	p_party_supported,
	v_dr_cr_flag,
    v_txn_amt,---p_orgl_tran_amt  
    v_orgl_auth_amt,--p_orgl_auth_amt
    v_fee_amount,--p_orgl_tranfee_amt
    v_closing_avail_balance,
	v_closing_ledger_balance,	--p_ledger_balance,----	
	 v_resp_code,	
    v_err_msg,
	 v_purse_id,
   v_avail_opening_balance,-- p_avail_opening_balance,
...
--UDPATE CARD STATUS AND TOP-UP FLAG 
        ...
        BEGIN
            UPDATE card
                SET
                    card_status=nvl(old_cardstat,trans_const.active),
                    firsttime_topup=v_firsttime_topup,
                    old_cardstat=trans_const.inactive
    ...
    --UPDATE ACCOUNT PURSE
        ...
        BEGIN
            ...
            FROM 
                account_purse
            where 
                account_id=p_account_id AND product_id=p_prod_id AND purse_id=v_purse_id;
        EXCEPTION
            WHEN no_data_found THEN
                v_resp_code:=trans_const.invalid_product_identifier;
            ...
            WHEN OTHERS THEN
                v_resp_code:=trans_const.invalid_request;
                p_err_msg:='EXCEPTION OCCURED WHILE GETTING BALANCE'
                ||substr(sqlerrm,1,255);
                RAISE l_exception;
        END;
...
--        BEGIN
--    --NEED TO CALL balance_auth_credit
--            
--            UPDATE account_purse
--                SET
--                    ledger_balance=nvl(v_ledger_balance,0),
--                    available_balance=nvl(v_available_balance,0)
--            WHERE account_id=p_account_id AND product_id=p_prod_id AND purse_id=v_purse_id;
--            
--            
--            p_ledger_balance := nvl(v_ledger_balance,0);
--            p_acct_balance := nvl(v_available_balance,0);
--            
----            balance_auth_credit(
----                p_prod_id,
----                p_trandate,
...
----            );
----
----            p_acct_balance:=v_closing_avail_balance;
----            p_ledger_balance:=v_closing_ledger_balance;
--
--         EXCEPTION
--            WHEN no_data_found THEN
--                v_resp_code:=trans_const.invalid_product_identifier;
...
---update location inventory 
 ...
 --LOCATION INVENTORY UPDATE ENDS   
    ...
    EXCEPTION
        ...
        WHEN OTHERS THEN
            p_resp_code := trans_const.system_error;
            p_err_msg := 'Error while card Deactivation exception '
             || substr(sqlerrm,1,200);
            ROLLBACK;
    END;   
