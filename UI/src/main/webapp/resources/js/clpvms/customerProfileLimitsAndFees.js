...
/*	function allowNumbersWithDot(ctrl){
	...
	function validateFeeAmt(ctrl){
		...
		if(feeAmount==null  || feeAmount.trim()==''){
			if($("#"+idPrefix+"_clawback").is(":checked") || $("#"+idPrefix+"_feeCondition option:selected").val()!='N' || 
			($("#"+idPrefix+"_freeCount").val()!=null && $("#"+idPrefix+"_freeCount").val().trim()!='') || 
			($("#"+idPrefix+"_maxCount").val()!=null && $("#"+idPrefix+"_maxCount").val().trim()!='') ){
		
				generateAlert($(ctrl).closest('form').attr('id'), ctrlId,"product.feeAmt.empty");
				$("#"+ctrlId.split('_', 1)+ctrlId.split('_', 1)).css('visibility',  'visible');
				isValid= false;
			}
			else{
		  ...
		  {
			if(isNaN(feeAmount) ){
				generateAlert($(ctrl).closest('form').attr('id'),  ctrlId,"product.feeAmt.invalid");
				$("#"+ctrlId.split('_', 1)+ctrlId.split('_', 1)).css('visibility',  'visible');
				isValid= false;
			}
			else if( parseFloat(feeAmount)<=0 ){
				generateAlert($(ctrl).closest('form').attr('id'),  ctrlId,"product.feeAmt.zero");
				$("#"+ctrlId.split('_', 1)+ctrlId.split('_', 1)).css('visibility',  'visible');
				isValid= false;
			}
			else if(!DecimalValueFormat(ctrl)){
