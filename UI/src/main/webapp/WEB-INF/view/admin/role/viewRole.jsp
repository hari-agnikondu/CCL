		...
		<ul class="nav nav-tabs col-lg-6 col-lg-offset-3">
			<li class="active"><a href="" data-toggle="tab"
				class="hoverColor"><i class="glyphicon glyphicon-tags"></i>&nbsp;<spring:message
						code="header.role.view" text="View Role" /></a></li>
			...
			<div class="tab-pane fade in active graybox col-lg-8 col-lg-offset-3" id="product">
				...
				<div class="form-inline">
					<form:form action="#" method="POST" modelAttribute="roleForm"
						id="roleFormView" name="roleFormView">
							...
							<div class="col-lg-12">
								...
								<div class="col-lg-8">
									<form:input title="Allowed Special Characters are ,;'_- ." path="roleName" class="form-group textbox-large trim"
										name="roleName" id="roleName" autocomplete="off" readonly="true"
							...
							<div class="col-lg-12" style="padding-bottom: 5px">
								...
								<div class="col-lg-8">
									<form:textarea 
										...
										cols="51" readonly="true" onblur="validateFields(this.form.id,this.id)"
										 style="resize:none"/>
						...
						<c:forEach items="${permissionMap}" var="permDto" >
							<tr><td>${permDto.key }<c:set var = "opMap"  value = "${permDto.value}"/></td>
								<td><c:if test="${ not empty opMap['Search']}">
									<form:checkbox  name="Search" id="search${opMap['Search']}" path="permissionID" value="${opMap['Search']}" disabled="true"/>
								</c:if></td>
								<td><c:if test="${not empty opMap['Add']}">
									<form:checkbox  name="Add" id="add${opMap['Add']}" path="permissionID" value="${opMap['Add']}" disabled="true"/>
								</c:if></td>
								<td><c:if test="${not empty opMap['Edit']}">
										<form:checkbox  name="Edit" id="edit${opMap['Edit']}" path="permissionID" value="${opMap['Edit']}" disabled="true"/>
								</c:if></td>
									<td><c:if test="${not empty opMap['View']}">
										<form:checkbox  name="View" id="view${opMap['View']}" path="permissionID" value="${opMap['View']}" disabled="true"/>
								</c:if></td>
								<td><c:if test="${not empty opMap['Delete']}">
									<form:checkbox  name="Delete" id="delete${opMap['Delete']}" path="permissionID" value="${opMap['Delete']}" disabled="true"/>
								</c:if></td>
								<td><c:if test="${not empty opMap['Download']}">
                                    <form:checkbox  name="Download" id="download${opMap['Download']}" path="permissionID" value="${opMap['Download']}" disabled="true"/>
                                </c:if></td>
								<td><c:if test="${not empty opMap['Approve']}">
									<form:checkbox  name="Approve" id="search${opMap['Approve']}" path="permissionID" value="${opMap['Approve']}" disabled="true"/>
								</c:if></td>
								<td><c:if test="${not empty opMap['Menu']}">
									<form:checkbox  name="Menu" id="menu${opMap['Menu']}" path="permissionID" value="${opMap['Menu']}" disabled="true"/>
								</c:if></td>
							</tr></c:forEach>
						</tbody>
