<!doctype html>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@taglib uri="http://www.springframework.org/tags" prefix="spring"%>
<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form"%>
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"%>
<%@page import="org.springframework.web.servlet.support.RequestContextUtils"%>
<%@ taglib prefix="security" uri="http://www.springframework.org/security/tags" %>



<script src="${pageContext.request.contextPath}/resources/js/clpvms/merchantProduct.js"></script>

<script src="${pageContext.request.contextPath}/resources/js/clpvms/user.js"></script>

<!-- delete box starts -->

<body>

<c:if test="${successstatus !=''}">
						<div class="col-lg-11 success-green  text-center padding" id="message" >
						<b><strong>${successstatus}</strong></b>
					</div>
			
			</c:if> 
			<c:if test="${failstatus !=''}">
			
					
					<div id="errmessage" class="error-red col-lg-11 text-center padding">
						<b><strong>${failstatus}</strong></b>
					</div>
				
			</c:if>
<div class="modal fade" id="define-constant-delete" tabindex="-1"
			role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				 <form:form commandName="deleteBox" name="deleteMerchantForm"
					id="deleteMerchantForm" method="post">
					<div class="modal-content">
						<div class="modal-body col-lg-12">
							<div class="col-lg-12">
								<span >
	Do you want to delete the record  Product "<b id="productNameDisp"></b>" mapped for Merchant "<b id="merchantNameDisp"></b>"  ? 
								</span>
							</div>

						</div>
				<input type="hidden" name="merchantId" id="merchantIdtoDelete" /> 
				<form:hidden path="merchantName" id="merchantNametoDelete" />
				<input type="hidden" name="productId" id="productIdtoDelete" /> 
				<form:hidden path="productName" id="productNametoDelete" />
						 
						 
						<div class="modal-footer">
							<button type="button" onclick="goDeleteProductMerchant();"
								class="btn btn-primary"><i class="glyphicon glyphicon-trash"></i><spring:message
									code="CardRange.deleteBtn" /></button>
							<button data-dismiss="modal" onclick="goToPrevious()" class="btn btn-primary gray-btn"><spring:message
									code="button.cancel" /></button>

						</div>

					</div>
				</form:form> 
			</div>
		</div>

<!-- delete box ends -->



	<div class="">


	<%-- 	<div class="aln_iss">
		<span id='hidemydata'> <c:if test="${successstatus !=''}">
				<h4>
					<div class="aln_iss_green">
						<b><font size="3"><center style="color: green;">
									<strong>${successstatus}</strong>
								</center></font></b>
					</div>
				</h4>
			</c:if> <c:if test="${failstatus !=''}">
				<h4>
					<div class="aln_iss_red">
						<b><font size="3"><center style="color: red;">
									<strong>${failstatus}</strong>
								</center></font></b>
					</div>
				</h4>
			</c:if>
		</span>
		</div> --%>
		<!-- graybox -->
		<!-- <div class="graybox col-lg-5 form-col-6 sb3"> -->
<div class="body-container" style="min-height: 131px;">   
<div class="container">

	<article class="col-lg-12"> 
		<div class="graybox col-lg-5 col-lg-offset-3 ">

			<form:form id="merchantToProductForm" method="post" commandName="merchantToProductForm" >

			 	<div class="col-lg-12">
					<h3>
						<spring:message code="merchant.searchAssignMerchantToProduct" />
					</h3>
				</div> 
				

				<!-- <div class="form"> -->

					<div class="col-lg-12">
                     <div class="">
                     
                     <div class="col-lg-12">
						<div class="col-lg-4">
							<label for="search_requesterName"> <spring:message
									code="merchant.enterProductName" /><font color='red'></font></label>

						</div>



						<div class="col-lg-8">

							<form:input  path="productName" id="productNameId"
								class="textbox xlarge4label trim" name='search_productName'
										onkeyup="return isAlphaNumericWithSpaceproductNameId(this)"  
									onblur="return validatewithmaxLengthMerchantProduct(this.form.id,this.id)"
								 type="textarea" maxlength="100" />
							<font color="red"><span id="productNameIdError"> </span></font>
							<div>
								<form:errors path="productName" id="productNameId"
									class="textbox xlarge4label" name='search_productName'
									cssStyle="color:red" />
							</div>

						</div>
						
						</div>
						
						<div class="col-lg-12">
						<div class="col-lg-12 text-right">
								<button type="submit" onclick="searchMerchantLinkedWithProduct(event)"
									class="btn btn-primary" id="search_submit">
									<i class="glyphicon glyphicon-search"></i>
									<spring:message code="button.search" />
								</button>
							
						</div>
			  </div>
				<div class="col-lg-12 ">
							<div class="col-lg-4 ">
							<label for="search_requesterName"> <spring:message
									code="merchant.enterMerchantName" /><font color='red'></font></label>

						</div>
						
					
				
							<div class="col-lg-8">

							<form:input  path="merchantName" id="merchantNameId"
								class="textbox xlarge4label trim" name='search_issuerName'
								onkeyup="return isAlphaNumericMerchantNameId(this)"
								onblur="return validatewithmaxLengthMerchantProduct(this.form.id,this.id)"
								 type="textarea" maxlength="100" />
							<font color="red"><span id="merchantNameIdError"> </span></font>
							<div>
								<form:errors path="merchantName" id="merchantNameId"
									class="textbox xlarge4label" name='search_merchantName'
									cssStyle="color:red" />
							</div>

						</div>
						</div>

						<!-- <div class=""> -->
							<div class="col-lg-12 text-center">
							
								<label> <spring:message code="merchantToProduct.hint" /></label>
							</div>
						<!-- </div> -->

						<form:hidden path="merchantId" id="merchantId" name="merchantId" value="" /> 
						<form:hidden path="productId" id="productId" name="productId"  value="" /><input
							id="searchType" name="searchType" type="hidden"
							value="${SearchType}" /> <input id="searchedName"
							name="searchedName" type="hidden" value="" /> <input
							id="retrievedName" name="retrievedName" type="hidden"
							value="${SearchedName}" /> <input id="deletedName"
							name="deletedName" type="hidden" value="" /> <input
							type="hidden" name="jsPath" id="jsPath"
							value="${pageContext.request.contextPath}/resources/JS_Messages/" />
					</div>
					<br />
</div>
			
		
		
		
		</form:form>
		</div>
		</article>

	</div>


<article class="col-lg-12">
<div class="col-lg-12 text-right">
				<security:authorize access="hasRole('ADD_MERCHANT_PRODUCT_LINK')">
					<button type="button" class="btn btn-primary"
						style="bottom: 15px; position: relative;"
						onclick="clickShowPrdtoMer()">
						<i class='glyphicon glyphicon-plus'></i>
						<spring:message code="merchant.assignPrdtoMer" />
					</button>
				</security:authorize>
</div>
</article>



<c:if test="${showGrid =='true' }">




	<div class="group">

		<h3>
			<i class="icon-table"></i>
		</h3>

		<div id="tableDiv">
			<!-- <table id="tableView" class="table table-hover table-striped table-bordered" style="width: 100% !important;"> -->
			<table id=""
				class="table table-hover table-striped table-bordered dataTable datagridwithsearch"
				style="width: 100% !important;">
				


				<thead class="table-head">
					<tr>
						
						<th><spring:message code="merchantProduct.productName" /></th>
						<th><spring:message code="merchantProduct.merchantName" /></th>
						
						<th style="width:22em"><spring:message code="merchantProduct.action" /></th>
 
					</tr>
				</thead>
				<tbody class="row">

					<c:forEach items="${merchantProductTableList}" var="merchantProductDetails"
						varStatus="status">
						
							
						<tr id="${merchantProductDetails.merchantId}">
				
						
						<td>${merchantProductDetails.productId}:${merchantProductDetails.productName}</td>
						<td>${merchantProductDetails.merchantName}</td>
					
						
									
							<td>
							<security:authorize access="hasRole('VIEW_MERCHANT_PRODUCT_LINK')">	
									<button type="button" class="btn btn-primary-table-button"
										onclick="clickViewForProdAcess(${merchantProductDetails.merchantId}+'-'+${merchantProductDetails.productId});">
										<i class="glyphicon glyphicon-list"></i>
										<spring:message code="groupAccessId.viewBtn" />
									</button>
									</security:authorize>
							
								<c:set var = "string1" value = "${merchantProductDetails.merchantName}"/>

			<c:set var = "string2" value ="${fn:replace(string1,'\\'','*')} "/> 
		<security:authorize access="hasRole('DELETE_MERCHANT_PRODUCT_LINK')">						
	<button type="submit" class="btn btn-primary-table-button" id="search_submit" data-toggle="modal" data-target="#define-constant-delete"
		onclick="goConfirmDeleteProductMerchant('${merchantProductDetails.merchantId}~${string2}#${merchantProductDetails.productId}~${merchantProductDetails.productName}')">
		<i class='glyphicon glyphicon-trash'></i>
		<spring:message code="button.delete" />
	</button>
	</security:authorize>
								
							
								
								
							</td>

						</tr>
						
					
					</c:forEach>
				</tbody>
			</table>
		</div>

		<div id="tableViewUsersStatus">
			<br>
		</div>

	</div>

</div>
</div>

</c:if>

</section>
</div>

</body>