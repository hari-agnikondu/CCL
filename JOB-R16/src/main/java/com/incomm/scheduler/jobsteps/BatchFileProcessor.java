...
@Component
public class BatchFileProcessor implements ItemProcessor<CnShipmentFile, CnShipmentFile>
{
	private static final Logger logger = LogManager.getLogger(BatchFileProcessor.class);
	@Autowired
	...
	@Override
	public CnShipmentFile process(CnShipmentFile items) throws Exception {
		try {
			...
			if(items.getSerialNumber()==null || items.getSerialNumber().length()==0 ||  items.getSerialNumber().length()>40)
			{
			...
			if(items.getOrderID()==null || items.getOrderID().length()==0)
			{
				throw new ServiceException("Invalid order_id");
			}
			...
			logger.debug("Log:"+items.getFileName()+"CN FILE UPLOAD "+items.getFileRecode()+" "+e.getMessage());
