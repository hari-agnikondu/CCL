...
/**
 * PostBackJob provides to send the postback request based on configuration
 */
...
@Service("com.incomm.scheduler.job.PostBackJob")
public class PostBackJob implements SchedulerJob {
	// the logger
	private static final Logger logger = LogManager.getLogger(PostBackJob.class);
	@Autowired
	...
	@Autowired
	@Qualifier("PostBackRetryJob")
	Job postBackRetryJob;
	...
	public void start(ProcessSchedule processSchedule) {
		future = scheduler.schedule(()->{
			...
			JobParameters jobParameters = new JobParametersBuilder()
					...
					.addString(JobConstants.JOBNAME, JobConstants.POSTBACK_JOB_ID).toJobParameters();
			JobExecution execution;
			String exitStatus = "";
			try {
				execution = jobLauncher.run(postBackRetryJob, jobParameters);
				exitStatus = execution.getStatus().toString();
				logger.info("*******exitStatus***"+exitStatus);
			} catch (JobExecutionAlreadyRunningException | JobRestartException
					| JobInstanceAlreadyCompleteException
					| JobParametersInvalidException e) {
				logger.error("Exception in start method of PostBackJob:"+e.getMessage());
			}
			finally{
				...
				if("COMPLETED".equals(exitStatus) && processSchedule.getMailSuccess()!=null ){
					notificationService.sendEmailNotification(processSchedule.getMailSuccess(),JobConstants.POSTBACK_JOB_ID,"Postback job completed Successfully");
				}
				else if("FAILED".equals(exitStatus) && processSchedule.getMailFail()!=null){
					notificationService.sendEmailNotification(processSchedule.getMailFail(),JobConstants.POSTBACK_JOB_ID,"Postback job process Failed");
				}
			}			
		},triggerContext->{
