...
@Service("com.incomm.scheduler.job.DormancyFeeJob")
public class DormancyFeeJob implements SchedulerJob{

	
	private static final Logger logger = LogManager.getLogger(DormancyFeeJob.class);
	...
	@Autowired
	@Qualifier("DormancyFeeJob")
	Job dormancyFeeJobBeen;
	...
	public void start(ProcessSchedule processSchedule) {
		...
		future = scheduler.schedule(() -> {
			...
			JobParameters jobParameters = new JobParametersBuilder()
					...
					.addString(JobConstants.JOBNAME, JobConstants.DORMANCY_FEE_JOB).toJobParameters();
			JobExecution execution;
			String exitStatus = "";
			try {
				execution = jobLauncher.run(dormancyFeeJobBeen, jobParameters);
				exitStatus = execution.getStatus().toString();
				logger.info("*******exitStatus***" + exitStatus);
			} catch (JobExecutionAlreadyRunningException | JobRestartException | JobInstanceAlreadyCompleteException
					| JobParametersInvalidException e) {
				logger.error("Exception in start method of Dormancy Fee Job:" + e.getMessage());
			} finally {
				...
				if("COMPLETED".equals(exitStatus) && processSchedule.getMailSuccess()!=null ){
					notificationService.sendEmailNotification(processSchedule.getMailSuccess(),JobConstants.DORMANCY_FEE_JOB,"Dormancy Fee Job completed Successfully");
					logger.info("Dormancy Fee Job status : "+exitStatus);
				}
				else if("FAILED".equals(exitStatus) && processSchedule.getMailFail()!=null){
					notificationService.sendEmailNotification(processSchedule.getMailFail(),JobConstants.DORMANCY_FEE_JOB,"Dormancy Fee Job completion Failed");
					logger.info("Dormancy Fee Job status : "+exitStatus);
				}
			}
		}, triggerContext -> {
