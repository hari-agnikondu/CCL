...
@Service("com.incomm.scheduler.job.CurrencyRateUploadJob")
public class CurrencyRateUploadJob implements SchedulerJob {
	// the logger
	private static final Logger logger = LogManager.getLogger(CurrencyRateUploadJob.class);
	@Autowired
	...
	public void start(ProcessSchedule processSchedule) {
		future = scheduler.schedule(()->{
			...
			try {
				...
				execution = jobLauncher.run(currencyRateUploadJob, jobParameters);
				exitStatus = execution.getStatus().toString();
				logger.info("*******exitStatus***"+exitStatus);
			} catch (JobExecutionAlreadyRunningException | JobRestartException
					| JobInstanceAlreadyCompleteException
					| JobParametersInvalidException e) {
				logger.error("Exception in start method of CurrencyRate Upload Job :"+e.getMessage());
			}
			finally{
