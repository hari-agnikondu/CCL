...
@Service("com.incomm.scheduler.job.WeeklyFeeJob")
public class WeeklyFeeJob implements SchedulerJob {

	private static final Logger logger = LogManager.getLogger(WeeklyFeeJob.class);
	...
	public void start(ProcessSchedule processSchedule) {
		...
		future = scheduler.schedule(() -> {
			...
			JobParameters jobParameters = new JobParametersBuilder()
					...
					.addString(JobConstants.JOBNAME, JobConstants.WEEKLY_FEE_JOB).toJobParameters();
			JobExecution execution;
			String exitStatus = "";
			try {
				execution = jobLauncher.run(weeklyFee, jobParameters);
				...
				if("COMPLETED".equals(exitStatus) && processSchedule.getMailSuccess()!=null ){
					...
					logger.info("Weekly Fee Job status : "+exitStatus);
				}
