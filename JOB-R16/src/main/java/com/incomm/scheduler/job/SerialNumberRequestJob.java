...
@Service("com.incomm.scheduler.job.SerialNumberRequestJob")
public class SerialNumberRequestJob implements SchedulerJob {


	private static final Logger logger = LogManager.getLogger(SerialNumberRequestJob.class);
	...
	@Autowired
	@Qualifier("serialNumberRequest")
	Job serialNumberRequest;
	...
	public void start(ProcessSchedule processSchedule) {
		...
		future = scheduler.schedule(() -> {
			...
			JobParameters jobParameters = new JobParametersBuilder()
					...
					.addString(JobConstants.JOBNAME, JobConstants.SERIAL_NUMBER_REQUEST_JOB).toJobParameters();
			JobExecution execution;
			String exitStatus = "";
			try {
				execution = jobLauncher.run(serialNumberRequest, jobParameters);
				exitStatus = execution.getStatus().toString();
				logger.info("*******exitStatus***" + exitStatus);
			} catch (JobExecutionAlreadyRunningException | JobRestartException | JobInstanceAlreadyCompleteException
					| JobParametersInvalidException e) {
				logger.error("Exception in start method of Weekly Fee Job:" + e.getMessage());
			} finally {
				...
				if("COMPLETED".equals(exitStatus) && processSchedule.getMailSuccess()!=null ){
					notificationService.sendEmailNotification(processSchedule.getMailSuccess(),JobConstants.WEEKLY_FEE_JOB,"Weekly Fee Job completed Successfully");
					logger.info("serialNumberRequest Job status : "+exitStatus);
				}
				else if("FAILED".equals(exitStatus) && processSchedule.getMailFail()!=null){
					notificationService.sendEmailNotification(processSchedule.getMailFail(),JobConstants.WEEKLY_FEE_JOB,"Weekly Fee Job completion Failed");
					logger.info("serialNumberRequest Job status : "+exitStatus);
				}
			}
		}, triggerContext -> {
