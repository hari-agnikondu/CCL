...
/**
 * ReturnFileJob provides to start the Return File Jobs based on configuration
...
@Service("com.incomm.scheduler.job.ReturnFileJob")
public class ReturnFileJob implements SchedulerJob {
	// the logger
	private static final Logger logger = LogManager.getLogger(ReturnFileJob.class);
	@Autowired
	...
	public void start(ProcessSchedule processSchedule) {
		...
		future = scheduler.schedule(()->{
			...
			JobParameters jobParameters = new JobParametersBuilder()
					...
					.addString(JobConstants.JOBNAME, JobConstants.RETURN_FILE_JOB_ID)
					.addString(JobConstants.DIRECTORY_PATH,returnFilePath)
					.toJobParameters();
			JobExecution execution;
			String exitStatus = "";
			try {
				execution = jobLauncher.run(returnFileJobBeen, jobParameters);
				exitStatus = execution.getStatus().toString();
				logger.info("*******exitStatus***"+exitStatus);
			} catch (JobExecutionAlreadyRunningException | JobRestartException
					| JobInstanceAlreadyCompleteException
					| JobParametersInvalidException e) {
				logger.error("Exception in start method of returnFileJob:"+e.getMessage());
			}
			finally{
				...
				if("COMPLETED".equals(exitStatus) && processSchedule.getMailSuccess()!=null ){
					notificationService.sendEmailNotification(processSchedule.getMailSuccess(),JobConstants.RETURN_FILE_JOB_ID,"Return File Uploaded Successfully");
				}
				else if("FAILED".equals(exitStatus) && processSchedule.getMailFail()!=null){
					notificationService.sendEmailNotification(processSchedule.getMailFail(),JobConstants.RETURN_FILE_JOB_ID, "Return File Upload Process Failed");
				}
			}
		},triggerContext->{
