...
@Service("com.incomm.scheduler.job.AutoReplenishmentInventoryGenerationJob")
public class AutoReplenishmentInventoryGenerationJob implements SchedulerJob{

	private static final Logger logger = LogManager.getLogger(AutoReplenishmentInventoryGenerationJob.class);
	...
	@Autowired
	 @Qualifier("AutoReplenishmentInventoryGenJob")
	Job autoReplenishmentInventoryGenJob;
	...
	public void start(ProcessSchedule processSchedule) {
		...
		future = scheduler.schedule(() ->{
			...
			JobParameters jobParameters = new JobParametersBuilder()
					...
					.addString(JobConstants.JOBNAME, JobConstants.AUTO_REPLENISHMENT_OF_INVENTORY_GEN_ID)
					.toJobParameters();
			...
			try {
				...
				execution = jobLauncher.run(autoReplenishmentInventoryGenJob, jobParameters);
				exitStatus = execution.getStatus().toString();
			} catch(JobExecutionAlreadyRunningException | JobRestartException
					| JobInstanceAlreadyCompleteException
					| JobParametersInvalidException e) {
				
				logger.error("Exception in start method of CardNumberInventoryGenerationJob:"+e);
			}
			finally{
				...
				if("COMPLETED".equals(exitStatus) && processSchedule.getMailSuccess()!=null ){
					notificationService.sendEmailNotification(processSchedule.getMailSuccess(),JobConstants.AUTO_REPLENISHMENT_OF_INVENTORY_GEN_ID,"Auto Replenishment of Inventory generated Successfully");
				}
				else if("FAILED".equals(exitStatus) && processSchedule.getMailFail()!=null){
					notificationService.sendEmailNotification(processSchedule.getMailFail(),JobConstants.AUTO_REPLENISHMENT_OF_INVENTORY_GEN_ID,"Auto Replenishment of Inventory generation Failed");
				}
			}
