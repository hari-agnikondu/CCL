...
@Repository
public class EODPreAuthReleaseDAOImpl  extends JdbcDaoSupport implements EODPreAuthReleaseDAO {
	
private static final Logger log = LogManager.getLogger(EODPreAuthReleaseDAOImpl.class);
	...
	@Override
	public String callEODPreAuthReleaseProcedure() {
		try {
			List<SqlParameter> params = new ArrayList<>();
			params.add(new SqlOutParameter("error", Types.VARCHAR));

			log.info("Calling the Procedure : {}", ScriptUtils.SP_EOD_PREAUTH_RELEASE);

			Map<String, Object> resultMap = getJdbcTemplate().call(new CallableStatementCreator() {
				public CallableStatement createCallableStatement(Connection con) throws SQLException {
					CallableStatement cs = con.prepareCall(ScriptUtils.SP_EOD_PREAUTH_RELEASE);
