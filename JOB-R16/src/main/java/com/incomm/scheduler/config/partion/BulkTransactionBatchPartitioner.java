...
public class BulkTransactionBatchPartitioner implements Partitioner {

	@Value("${SPLIT_VALUE}")
	private Integer splitvalue;
	
	private static final Logger logger = LogManager.getLogger(BulkTransactionBatchPartitioner.class);
	private Map<String, String> fileList;
	private final BulkTransactionDAO bulkTransactionDao;

	public BulkTransactionBatchPartitioner(BulkTransactionDAO bulkTransactionDao) {
	...
	public Map<String, ExecutionContext> partition(int gridSize) {
		...
		for (Entry<String, String> batch : fileList.entrySet()) {
			...
			try {
				int count = bulkTransactionDao.getRecordCountBatchId(Long.parseLong(batch.getKey()));
