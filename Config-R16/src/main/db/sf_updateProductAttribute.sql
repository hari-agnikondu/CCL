/*
	Updates JSON product attributes using Oracle's support for managing JSON structure.
	
	The input parameters are:
	prod_id - the id of the product for which the attributes need to be updated.
	attribute_name - the name of the attribute which needs to be updated.
	attribute_value - the new value of the attribute. This is only used for ADD/ UPDATE operations.
	attribute_group - the parent group the attribute belongs to. 
	operation - the operation to be performed. Possible values are ADD, SELECT, UPDATE and DELETE.
*/

CREATE OR REPLACE FUNCTION sf_updateProductAttribute(
    prod_id IN NUMBER,
...
SET SERVEROUTPUT ON;
select sf_updateProductAttribute(
12,
'1_delchannel_tx_maxCountFreq_ovrd',
'false',
'Transaction Fees',
'ADD'
) from dual;
